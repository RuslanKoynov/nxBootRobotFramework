...
def parse_city():
    with open("C:\Новая папка\OpenWeater\libraries\city.list.json", "r") as read_file:
...
def get_day_length():
    ...
    for i in list_tmp:
        ...
        if day_length > condition:
            day_length_time = str(
                '{:02}:{:02}:{:02}'.format(day_length // 3600 % 24, day_length % 3600 // 60, day_length % 3600 % 60))
...
def get_humidity_and_pressure():
    ...
    for i in list_tmp:
        ...
        if (humidity > condition_humidity) and (pressure > condition_pressure):
            pair[i.get_location().get_name()] = 'humidity: ' + str(humidity) + '% pressure: ' + str(
                pressure) + ' мм.рт.ст'
    if pair:
        return pair, status
    else:
